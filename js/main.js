...
//   d3.select("#byId").attr('height', '500px').attr('width', '500px').style('background', 'red').selectAll("p").data(dataset.data).enter().append("p")
//     .text('text');
...
//   const data = await d3.json("data/gdp.json");
// console.log(dataset);
...
const x = d3
  .scaleUtc()
  .domain([ d3.min(dataset.data, d=>new Date(d[0])) , d3.max(dataset.data, d=>new Date(d[0]))])
...
svg
  ...
  .attr("id", "title")
  .text("Gross Domestic Product in the US"); // Replace with your desired title

  d3.select('#container')
  .node()  // Select the raw DOM node
  .appendChild(svg.node());  // Append the SVG DOM node to the container
